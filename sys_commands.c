...
int cmd_help(ShellContext *ctx, int argc, char *argv[], FILE *in, FILE *out) {
    ...
    CommandInfo commands[] = {
        ...
        },
        {
            ...
            "  在指定目录及其子目录中查找文件或目录。\n"
            "  选项:\n"
            "    -name 模式    按名称精确匹配（区分大小写）\n"
            "    -iname 模式   按名称部分匹配（不区分大小写）\n"
            "    -type 类型    按类型查找，d=目录，f=文件\n"
            "    -count        只显示匹配项的数量\n"
            "  示例:\n"
            "    find -name test.txt          在当前目录查找名为test.txt的文件\n"
            "    find /home -name *.txt       在/home目录下查找所有.txt文件\n"
            "    find -iname test             在当前目录查找包含\"test\"的文件（不区分大小写）\n"
            "    find -type d                 在当前目录查找所有目录\n"
            "    find -name *.txt -count      统计当前目录下.txt文件的数量\n"
        },
        {
            "help",
            "显示帮助信息",
